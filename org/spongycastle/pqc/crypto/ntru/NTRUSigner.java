...
public class NTRUSigner {
    ...
    private byte[] signHash(byte[] bArr, NTRUSigningPrivateKeyParameters nTRUSigningPrivateKeyParameters) {
        ...
        } while (!verify(createMsgRep, sign, publicKey.f1438h));
        byte[] binary = sign.toBinary(this.params.f1435q);
    ...
    private IntegerPolynomial sign(IntegerPolynomial integerPolynomial, NTRUSigningPrivateKeyParameters nTRUSigningPrivateKeyParameters) {
        int i = this.params.f1430N;
        int i2 = this.params.f1435q;
        NTRUSigningPublicKeyParameters publicKey = nTRUSigningPrivateKeyParameters.getPublicKey();
        IntegerPolynomial integerPolynomial2 = new IntegerPolynomial(i);
        for (int i3 = this.params.f1429B; i3 >= 1; i3--) {
            Polynomial polynomial = nTRUSigningPrivateKeyParameters.getBasis(i3).f1436f;
            ...
            IntegerPolynomial integerPolynomial3 = (IntegerPolynomial) nTRUSigningPrivateKeyParameters.getBasis(i3).f1437h.clone();
            if (i3 > 1) {
                integerPolynomial3.sub(nTRUSigningPrivateKeyParameters.getBasis(i3 - 1).f1437h);
            } else {
                integerPolynomial3.sub(publicKey.f1438h);
            }
            integerPolynomial = mult2.mult(integerPolynomial3, i2);
        }
        Polynomial polynomial3 = nTRUSigningPrivateKeyParameters.getBasis(0).f1436f;
    ...
    private boolean verifyHash(byte[] bArr, byte[] bArr2, NTRUSigningPublicKeyParameters nTRUSigningPublicKeyParameters) {
        ...
        wrap.get(bArr3);
        return verify(createMsgRep(bArr, wrap.getInt()), IntegerPolynomial.fromBinary(bArr3, this.params.f1430N, this.params.f1435q), nTRUSigningPublicKeyParameters.f1438h);
    }
    ...
    public IntegerPolynomial createMsgRep(byte[] bArr, int i) {
        int i2 = this.params.f1430N;
        int numberOfLeadingZeros = 31 - Integer.numberOfLeadingZeros(this.params.f1435q);
