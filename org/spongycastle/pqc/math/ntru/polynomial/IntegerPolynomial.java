...
public class IntegerPolynomial implements Polynomial {
    ...
    private IntegerPolynomial mod2ToModq(IntegerPolynomial integerPolynomial, int i) {
        int i2 = 2;
        if (!C3249Util.is64BitJVM() || i != 2048) {
            while (i2 < i) {
                i2 *= 2;
    ...
    public Resultant resultant() {
        ...
        while (true) {
            ...
            mod = bigInteger2.multiply(calculate.f1503x.multiply(bigInteger3)).add(resultant.res.multiply(calculate.f1504y.multiply(bigInteger))).mod(multiply);
    ...
    public ModularResultant resultant(int i) {
        ...
        while (degree > 0) {
            int invert = (C3249Util.invert(integerPolynomial10.coeffs[degree], i) * integerPolynomial9.coeffs[i5]) % i;
            int i6 = i5 - degree;
            ...
            if (i5 < degree) {
                i4 = (i4 * C3249Util.pow(integerPolynomial10.coeffs[degree], i3 - i5, i)) % i;
        ...
        int pow = (i4 * C3249Util.pow(integerPolynomial10.coeffs[0], i5, i)) % i;
        integerPolynomial7.mult(C3249Util.invert(integerPolynomial10.coeffs[0], i));
