...
public class Tables8kGCMMultiplier implements GCMMultiplier {
    ...
    public void init(byte[] bArr) {
        if (this.f1107M == null) {
            this.f1107M = (int[][][]) Array.newInstance(int.class, new int[]{32, 16, 4});
        } else if (Arrays.areEqual(this.f1106H, bArr)) {
            return;
        }
        this.f1106H = Arrays.clone(bArr);
        GCMUtil.asInts(bArr, this.f1107M[1][8]);
        for (int i = 4; i >= 1; i >>= 1) {
            int[][][] iArr = this.f1107M;
            GCMUtil.multiplyP(iArr[1][i + i], iArr[1][i]);
        }
        int[][][] iArr2 = this.f1107M;
        int i2 = 0;
        GCMUtil.multiplyP(iArr2[1][1], iArr2[0][8]);
        for (int i3 = 4; i3 >= 1; i3 >>= 1) {
            int[][][] iArr3 = this.f1107M;
            GCMUtil.multiplyP(iArr3[0][i3 + i3], iArr3[0][i3]);
        }
        while (true) {
            for (int i4 = 2; i4 < 16; i4 += i4) {
                for (int i5 = 1; i5 < i4; i5++) {
                    int[][][] iArr4 = this.f1107M;
    ...
    public void multiplyH(byte[] bArr) {
        ...
        for (int i = 15; i >= 0; i--) {
            int[][][] iArr2 = this.f1107M;
            int i2 = i + i;
            int[] iArr3 = iArr2[i2][bArr[i] & Ascii.f278SI];
