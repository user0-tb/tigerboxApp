...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    private void CTR_DRBG_Reseed_algorithm(EntropySource entropySource, byte[] bArr) {
        CTR_DRBG_Update(Block_Cipher_df(Arrays.concatenate(entropySource.getEntropy(), bArr), this._seedLength), this._Key, this.f1173_V);
        this._reseedCounter = 1;
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (bArr2 != null) {
            ...
            CTR_DRBG_Update(bArr3, this._Key, this.f1173_V);
        } else {
            bArr3 = new byte[this._seedLength];
        }
        int length = this.f1173_V.length;
        byte[] bArr4 = new byte[length];
        this._engine.init(true, new KeyParameter(expandKey(this._Key)));
        for (int i = 0; i < bArr.length / length; i++) {
            addOneTo(this.f1173_V);
            this._engine.processBlock(this.f1173_V, 0, bArr4, 0);
            int i2 = i * length;
            System.arraycopy(bArr4, 0, bArr, i2, bArr.length - i2 > length ? length : bArr.length - (this.f1173_V.length * i));
        }
        CTR_DRBG_Update(bArr3, this._Key, this.f1173_V);
        this._reseedCounter++;
