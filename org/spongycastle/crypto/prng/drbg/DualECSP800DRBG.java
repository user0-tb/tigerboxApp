...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    /* renamed from: _P */
    private ECPoint f1176_P;

    /* renamed from: _Q */
    private ECPoint f1177_Q;
    private ECCurve.C3204Fp _curve;
        ...
        if (Utils.isTooLarge(bArr, 512)) {
            ...
            while (true) {
                if (i2 == dualECPointsArr.length) {
                    ...
                    this.f1176_P = dualECPointsArr[i2].getP();
                    this.f1177_Q = dualECPointsArr[i2].getQ();
                } else {
                    throw new IllegalArgumentException("Requested security strength is not supported by digest");
                }
            }
            if (this.f1176_P != null) {
                byte[] hash_df = Utils.hash_df(this._digest, concatenate, this._seedlen);
                this.f1178_s = hash_df;
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (Utils.isTooLarge(bArr2, 512)) {
            ...
            if (bArr2 != null) {
                bigInteger = new BigInteger(1, xor(this.f1178_s, Utils.hash_df(this._digest, bArr2, this._seedlen)));
            } else {
                bigInteger = new BigInteger(1, this.f1178_s);
            }
            Arrays.fill(bArr, (byte) 0);
            int i = 0;
            for (int i2 = 0; i2 < length2; i2++) {
                bigInteger = getScalarMultipleXCoord(this.f1176_P, bigInteger);
                byte[] byteArray = getScalarMultipleXCoord(this.f1177_Q, bigInteger).toByteArray();
            ...
            if (i < bArr.length) {
                bigInteger = getScalarMultipleXCoord(this.f1176_P, bigInteger);
                byte[] byteArray2 = getScalarMultipleXCoord(this.f1177_Q, bigInteger).toByteArray();
            ...
            this.f1178_s = BigIntegers.asUnsignedByteArray(this._sLength, getScalarMultipleXCoord(this.f1176_P, bigInteger));
            return length;
        }
    }
    ...
    public void reseed(byte[] bArr) {
        if (!Utils.isTooLarge(bArr, 512)) {
            this.f1178_s = Utils.hash_df(this._digest, Arrays.concatenate(pad8(this.f1178_s, this._seedlen), this._entropySource.getEntropy(), bArr), this._seedlen);
            this._reseedCounter = 0;
            return;
        }
