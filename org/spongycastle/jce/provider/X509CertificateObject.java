...
public class X509CertificateObject extends X509Certificate implements PKCS12BagAttributeCarrier {
    ...
    public void checkValidity(Date date) throws CertificateExpiredException, CertificateNotYetValidException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f1233c.getEndDate().getTime());
        } else if (date.getTime() < getNotBefore().getTime()) {
            throw new CertificateNotYetValidException("certificate not valid till " + this.f1233c.getStartDate().getTime());
        }
    }

    public int getVersion() {
        return this.f1233c.getVersionNumber();
    }

    public BigInteger getSerialNumber() {
        return this.f1233c.getSerialNumber().getValue();
    }

    public Principal getIssuerDN() {
        try {
            return new X509Principal(X500Name.getInstance(this.f1233c.getIssuer().getEncoded()));
    ...
    public X500Principal getIssuerX500Principal() {
        try {
            ...
            new ASN1OutputStream(byteArrayOutputStream).writeObject(this.f1233c.getIssuer());
    ...
    public Principal getSubjectDN() {
        return new X509Principal(X500Name.getInstance(this.f1233c.getSubject().toASN1Primitive()));
    }

    public X500Principal getSubjectX500Principal() {
        try {
            ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
            new ASN1OutputStream(byteArrayOutputStream).writeObject(this.f1233c.getSubject());
            return new X500Principal(byteArrayOutputStream.toByteArray());
        } catch (IOException unused) {
            throw new IllegalStateException("can't encode issuer DN");
        }
    ...
    public Date getNotBefore() {
        return this.f1233c.getStartDate().getDate();
    }

    public Date getNotAfter() {
        return this.f1233c.getEndDate().getDate();
    }

    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f1233c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
    ...
    public byte[] getSignature() {
        return this.f1233c.getSignature().getBytes();
    }
    ...
    public String getSigAlgOID() {
        return this.f1233c.getSignatureAlgorithm().getAlgorithm().getId();
    }

    public byte[] getSigAlgParams() {
        if (this.f1233c.getSignatureAlgorithm().getParameters() != null) {
            try {
                return this.f1233c.getSignatureAlgorithm().getParameters().toASN1Primitive().getEncoded(ASN1Encoding.DER);
            } catch (IOException unused) {
            }
        }
        return null;
    ...
    public boolean[] getIssuerUniqueID() {
        DERBitString issuerUniqueId = this.f1233c.getTBSCertificate().getIssuerUniqueId();
    ...
    public boolean[] getSubjectUniqueID() {
        DERBitString subjectUniqueId = this.f1233c.getTBSCertificate().getSubjectUniqueId();
    ...
    public Set getCriticalExtensionOIDs() {
        ...
        Extensions extensions = this.f1233c.getTBSCertificate().getExtensions();
        if (extensions == null) {
            return null;
        }
    ...
    public boolean hasUnsupportedCriticalExtension() {
        ...
        if (getVersion() != 3 || (extensions = this.f1233c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration oids = extensions.oids();
        while (oids.hasMoreElements()) {
    ...
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f1233c.getSubjectPublicKeyInfo());
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public byte[] getEncoded() throws CertificateEncodingException {
        try {
            return this.f1233c.getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public final void verify(PublicKey publicKey) throws CertificateException, NoSuchAlgorithmException, InvalidKeyException, NoSuchProviderException, SignatureException {
        Signature signature;
        String signatureName = X509SignatureUtil.getSignatureName(this.f1233c.getSignatureAlgorithm());
        try {
    ...
    public final void verify(PublicKey publicKey, String str) throws CertificateException, NoSuchAlgorithmException, InvalidKeyException, NoSuchProviderException, SignatureException {
        checkSignature(publicKey, Signature.getInstance(X509SignatureUtil.getSignatureName(this.f1233c.getSignatureAlgorithm()), str));
    }

    private void checkSignature(PublicKey publicKey, Signature signature) throws CertificateException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        if (isAlgIdEqual(this.f1233c.getSignatureAlgorithm(), this.f1233c.getTBSCertificate().getSignature())) {
            X509SignatureUtil.setSignatureParameters(signature, this.f1233c.getSignatureAlgorithm().getParameters());
