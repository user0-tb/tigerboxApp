...
public final class ServerSideAdInsertionUtil {
    ...
    public static AdPlaybackState addAdGroupToAdPlaybackState(AdPlaybackState adPlaybackState, long j, long j2, long... jArr) {
        ...
        return correctFollowingAdGroupTimes(withContentResumeOffsetUs, i, C1229Util.sum(jArr), j2);
    }
    ...
    public static long getStreamPositionUs(Player player, AdPlaybackState adPlaybackState) {
        ...
        if (!C1229Util.areEqual(period.getAdsId(), adPlaybackState.adsId)) {
            return C0963C.TIME_UNSET;
        }
        if (!player.isPlayingAd()) {
            return getStreamPositionUsForContent(C1229Util.msToUs(player.getCurrentPosition()) - period.getPositionInWindowUs(), -1, adPlaybackState);
        }
        return getStreamPositionUsForAd(C1229Util.msToUs(player.getCurrentPosition()), player.getCurrentAdGroupIndex(), player.getCurrentAdIndexInAdGroup(), adPlaybackState);
    }
