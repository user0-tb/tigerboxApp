...
final class RtspMessageUtil {
    ...
    private static final String CRLF = new String(new byte[]{Ascii.f269CR, 10});
    public static final long DEFAULT_RTSP_TIMEOUT_MS = 60000;

    /* renamed from: LF */
    private static final String f177LF = new String(new byte[]{10});
    ...
    public static ImmutableList<String> serializeRequest(RtspRequest rtspRequest) {
        ...
        builder.add((Object) C1229Util.formatInvariant("%s %s %s", toMethodString(rtspRequest.method), rtspRequest.uri, RTSP_VERSION));
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < immutableList.size(); i++) {
                builder.add((Object) C1229Util.formatInvariant("%s: %s", next, immutableList.get(i)));
            }
        }
        builder.add((Object) "");
    ...
    public static ImmutableList<String> serializeResponse(RtspResponse rtspResponse) {
        ...
        ImmutableList.Builder builder = new ImmutableList.Builder();
        builder.add((Object) C1229Util.formatInvariant("%s %s %s", RTSP_VERSION, Integer.valueOf(rtspResponse.status), getRtspStatusReasonPhrase(rtspResponse.status)));
    ...
    public static byte[] convertMessageToByteArray(List<String> list) {
        return Joiner.m226on(CRLF).join((Iterable<? extends Object>) list).getBytes(RtspMessageChannel.CHARSET);
    }
    ...
    public static Uri removeUserInfo(Uri uri) {
        ...
        return uri.buildUpon().encodedAuthority(C1229Util.split(str, "@")[1]).build();
    }
    ...
    public static RtspAuthUserInfo parseUserInfo(Uri uri) {
        ...
        String[] splitAtFirst = C1229Util.splitAtFirst(userInfo, ":");
    ...
    public static RtspResponse parseResponse(List<String> list) {
        ...
        return new RtspResponse(parseInt, new RtspHeaders.Builder().addAll(list.subList(1, indexOf)).build(), Joiner.m226on(CRLF).join((Iterable<? extends Object>) list.subList(indexOf + 1, list.size())));
    }
    ...
    public static RtspRequest parseRequest(List<String> list) {
        ...
        Assertions.checkArgument(z);
        return new RtspRequest(parse, parseMethodString, new RtspHeaders.Builder().addAll(list.subList(1, indexOf)).build(), Joiner.m226on(CRLF).join((Iterable<? extends Object>) list.subList(indexOf + 1, list.size())));
    }
    ...
    public static String[] splitRtspMessageBody(String str) {
        ...
        return C1229Util.split(str, str2);
    }
    ...
    public static ImmutableList<Integer> parsePublicHeader(String str) {
        ...
        ImmutableList.Builder builder = new ImmutableList.Builder();
        for (String parseMethodString : C1229Util.split(str, ",\\s?")) {
