...
public class DefaultTimeBar extends View implements TimeBar {
    ...
    public DefaultTimeBar(Context context, AttributeSet attributeSet, int i, AttributeSet attributeSet2, int i2) {
        ...
        if (attributeSet3 != null) {
            Paint paint7 = paint4;
            Paint paint8 = paint5;
            TypedArray obtainStyledAttributes = context.getTheme().obtainStyledAttributes(attributeSet3, C1187R.styleable.DefaultTimeBar, i, i2);
            try {
                Drawable drawable = obtainStyledAttributes.getDrawable(C1187R.styleable.DefaultTimeBar_scrubber_drawable);
                this.scrubberDrawable = drawable;
                if (drawable != null) {
                    setDrawableLayoutDirection(drawable);
                    dpToPx2 = Math.max(drawable.getMinimumHeight(), dpToPx2);
                }
                this.barHeight = obtainStyledAttributes.getDimensionPixelSize(C1187R.styleable.DefaultTimeBar_bar_height, dpToPx);
                this.touchTargetHeight = obtainStyledAttributes.getDimensionPixelSize(C1187R.styleable.DefaultTimeBar_touch_target_height, dpToPx2);
                this.barGravity = obtainStyledAttributes.getInt(C1187R.styleable.DefaultTimeBar_bar_gravity, 0);
                this.adMarkerWidth = obtainStyledAttributes.getDimensionPixelSize(C1187R.styleable.DefaultTimeBar_ad_marker_width, dpToPx3);
                this.scrubberEnabledSize = obtainStyledAttributes.getDimensionPixelSize(C1187R.styleable.DefaultTimeBar_scrubber_enabled_size, dpToPx4);
                this.scrubberDisabledSize = obtainStyledAttributes.getDimensionPixelSize(C1187R.styleable.DefaultTimeBar_scrubber_disabled_size, dpToPx5);
                this.scrubberDraggedSize = obtainStyledAttributes.getDimensionPixelSize(C1187R.styleable.DefaultTimeBar_scrubber_dragged_size, dpToPx6);
                int i3 = obtainStyledAttributes.getInt(C1187R.styleable.DefaultTimeBar_played_color, -1);
                int i4 = obtainStyledAttributes.getInt(C1187R.styleable.DefaultTimeBar_scrubber_color, -1);
                int i5 = obtainStyledAttributes.getInt(C1187R.styleable.DefaultTimeBar_buffered_color, DEFAULT_BUFFERED_COLOR);
                int i6 = obtainStyledAttributes.getInt(C1187R.styleable.DefaultTimeBar_unplayed_color, DEFAULT_UNPLAYED_COLOR);
                int i7 = obtainStyledAttributes.getInt(C1187R.styleable.DefaultTimeBar_ad_marker_color, DEFAULT_AD_MARKER_COLOR);
                int i8 = obtainStyledAttributes.getInt(C1187R.styleable.DefaultTimeBar_played_ad_marker_color, DEFAULT_PLAYED_AD_MARKER_COLOR);
                paint.setColor(i3);
                paint6.setColor(i4);
                paint2.setColor(i5);
                paint3.setColor(i6);
        ...
        valueAnimator.addUpdateListener(new DefaultTimeBar$$ExternalSyntheticLambda0(this));
        this.duration = C0963C.TIME_UNSET;
        this.keyTimeIncrement = C0963C.TIME_UNSET;
    ...
    public /* synthetic */ void m733lambda$new$0$comgoogleandroidexoplayer2uiDefaultTimeBar() {
        stopScrubbing(false);
    }

    /* renamed from: lambda$new$1$com-google-android-exoplayer2-ui-DefaultTimeBar  reason: not valid java name */
    public /* synthetic */ void m734lambda$new$1$comgoogleandroidexoplayer2uiDefaultTimeBar(ValueAnimator valueAnimator) {
    ...
    public void setDuration(long j) {
        if (this.duration != j) {
            this.duration = j;
            if (this.scrubbing && j == C0963C.TIME_UNSET) {
                stopScrubbing(true);
            }
            update();
        }
    ...
    private boolean scrubIncrementally(long j) {
        ...
        long constrainValue = C1229Util.constrainValue(j3 + j, 0, j2);
    ...
    private void positionScrubber(float f) {
        this.scrubberBar.right = C1229Util.constrainValue((int) f, this.progressBar.left, this.progressBar.right);
    }
    ...
    private long getScrubberPosition() {
        if (this.progressBar.width() <= 0 || this.duration == C0963C.TIME_UNSET) {
            return 0;
        }
    ...
    private void drawTimeBar(Canvas canvas) {
        ...
        if (this.adGroupCount != 0) {
            ...
            for (int i5 = 0; i5 < this.adGroupCount; i5++) {
                int min = this.progressBar.left + Math.min(this.progressBar.width() - this.adMarkerWidth, Math.max(0, ((int) ((((long) this.progressBar.width()) * C1229Util.constrainValue(jArr[i5], 0, this.duration)) / this.duration)) - i4));
    ...
    private void drawPlayhead(Canvas canvas) {
        int i;
        if (this.duration > 0) {
            int constrainValue = C1229Util.constrainValue(this.scrubberBar.right, this.scrubberBar.left, this.progressBar.right);
    ...
    private String getProgressText() {
        return C1229Util.getStringForTime(this.formatBuilder, this.formatter, this.position);
    }
    ...
    private boolean setDrawableLayoutDirection(Drawable drawable) {
        return C1229Util.SDK_INT >= 23 && setDrawableLayoutDirection(drawable, getLayoutDirection());
    }

    private static boolean setDrawableLayoutDirection(Drawable drawable, int i) {
        return C1229Util.SDK_INT >= 23 && drawable.setLayoutDirection(i);
    }
}
