...
final class SsaStyle {
    ...
    public static SsaStyle fromStyleLine(String str, Format format) {
        ...
        if (split.length != format2.length) {
            Log.m157w(TAG, C1229Util.formatInvariant("Skipping malformed 'Style:' line (expected %s values, found %s): '%s'", Integer.valueOf(format2.length), Integer.valueOf(split.length), str2));
            return null;
        }
        try {
            ...
            Log.m158w(TAG, "Skipping malformed 'Style:' line: '" + str2 + "'", e);
            return null;
        }
    }
    ...
    public static int parseAlignment(String str) {
        ...
        Log.m157w(TAG, "Ignoring unknown alignment: " + str);
        return -1;
    }
    ...
    private static int parseBorderStyle(String str) {
        ...
        Log.m157w(TAG, "Ignoring unknown BorderStyle: " + str);
        return -1;
    }
    ...
    public static Integer parseColor(String str) {
        ...
        try {
            ...
            Log.m158w(TAG, "Failed to parse color expression: '" + str + "'", e);
            return null;
        }
    }
    ...
    private static float parseFontSize(String str) {
        try {
            ...
            Log.m158w(TAG, "Failed to parse font size: '" + str + "'", e);
    ...
    private static boolean parseBooleanValue(String str) {
        try {
            ...
            Log.m158w(TAG, "Failed to parse boolean value: '" + str + "'", e);
            return false;
        }
    }
    ...
    static final class Overrides {
        ...
        private static final Pattern MOVE_PATTERN = Pattern.compile(C1229Util.formatInvariant("\\\\move\\(%1$s,%1$s,(%1$s),(%1$s)(?:,%1$s,%1$s)?\\)", PADDED_DECIMAL_PATTERN));
        private static final String PADDED_DECIMAL_PATTERN = "\\s*\\d+(?:\\.\\d+)?\\s*";
        private static final Pattern POSITION_PATTERN = Pattern.compile(C1229Util.formatInvariant("\\\\pos\\((%1$s),(%1$s)\\)", PADDED_DECIMAL_PATTERN));
        ...
        private static PointF parsePosition(String str) {
            ...
            if (find) {
                if (find2) {
                    Log.m155i(TAG, "Override has both \\pos(x,y) and \\move(x1,y1,x2,y2); using \\pos values. override='" + str + "'");
                }
