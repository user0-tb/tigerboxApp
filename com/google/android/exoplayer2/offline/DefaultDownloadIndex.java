...
public final class DefaultDownloadIndex implements WritableDownloadIndex {
    ...
    private void putDownloadInternal(Download download, SQLiteDatabase sQLiteDatabase) {
        byte[] bArr = download.request.keySetId == null ? C1229Util.EMPTY_BYTE_ARRAY : download.request.keySetId;
        ContentValues contentValues = new ContentValues();
        contentValues.put("id", download.request.f165id);
    ...
    private List<Download> loadDownloadsFromVersion2(SQLiteDatabase sQLiteDatabase) {
        ...
        if (!C1229Util.tableExists(sQLiteDatabase, this.tableName)) {
            return arrayList;
        }
    ...
    private static List<StreamKey> decodeStreamKeys(String str) {
        ...
        for (String split : C1229Util.split(str, ",")) {
            String[] split2 = C1229Util.split(split, "\\.");
