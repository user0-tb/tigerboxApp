...
public final class CacheDataSink implements DataSink {
    ...
    public CacheDataSink(Cache cache2, long j, int i) {
        ...
        if (i2 != 0 && j < 2097152) {
            Log.m157w(TAG, "fragmentSize is below the minimum recommended value of 2097152. This may cause poor cache performance.");
        }
        ...
        if (dataSpec2 != null) {
            ...
            while (i3 < i2) {
                try {
                    ...
                    ((OutputStream) C1229Util.castNonNull(this.outputStream)).write(bArr, i + i3, min);
                    i3 += min;
                    long j = (long) min;
    ...
    private void openNextOutputStream(DataSpec dataSpec2) throws IOException {
        ...
        this.file = this.cache.startFile((String) C1229Util.castNonNull(dataSpec2.key), dataSpec2.position + this.dataSpecBytesWritten, j);
    ...
    private void closeCurrentOutputStream() throws IOException {
        ...
        if (outputStream2 != null) {
            try {
                outputStream2.flush();
                C1229Util.closeQuietly(this.outputStream);
                this.outputStream = null;
                this.file = null;
                this.cache.commitFile((File) C1229Util.castNonNull(this.file), this.outputStreamBytesWritten);
            } catch (Throwable th) {
                C1229Util.closeQuietly(this.outputStream);
                this.outputStream = null;
                this.file = null;
                ((File) C1229Util.castNonNull(this.file)).delete();
                throw th;
            }
        }
    }
