...
public final class FlacExtractor implements Extractor {
    ...
    private void readMetadataBlocks(ExtractorInput extractorInput) throws IOException {
        ...
        while (!z) {
            ...
            this.flacStreamMetadata = (FlacStreamMetadata) C1229Util.castNonNull(flacStreamMetadataHolder.flacStreamMetadata);
        }
        Assertions.checkNotNull(this.flacStreamMetadata);
        this.minFrameSize = Math.max(this.flacStreamMetadata.minFrameSize, 6);
        ((TrackOutput) C1229Util.castNonNull(this.trackOutput)).format(this.flacStreamMetadata.getFormat(this.streamMarkerAndInfoBlock, this.id3Metadata));
        this.state = 4;
    }
    ...
    private void getFrameStartMarker(ExtractorInput extractorInput) throws IOException {
        ...
        ((ExtractorOutput) C1229Util.castNonNull(this.extractorOutput)).seekMap(getSeekMap(extractorInput.getPosition(), extractorInput.getLength()));
        this.state = 5;
    }
    ...
    private void outputSampleMetadata() {
        ((TrackOutput) C1229Util.castNonNull(this.trackOutput)).sampleMetadata((this.currentFrameFirstSampleNumber * 1000000) / ((long) ((FlacStreamMetadata) C1229Util.castNonNull(this.flacStreamMetadata)).sampleRate), 1, this.currentFrameBytesWritten, 0, (TrackOutput.CryptoData) null);
    }
}
